...
- **モデルギャラリー**: リボンの「ギャラリー」リンクから、コミュニティが投稿したモデル(`models/`
  に追加)を静的なページ(`/gallery/`)で閲覧し、「開く」でそのままアプリに読み込めます。投稿は
  リボンの「ギャラリーに投稿」ボタン(GitHubアカウントのみで完結、Issueフォーム経由の自動PR)が
  簡単です(詳細は[CONTRIBUTING.md](./CONTRIBUTING.md#モデルギャラリーコミュニティモデル共有について)参照)
- **AI生成(APIキー不要)**: 自然言語プロンプトからパラメトリックモデルを生成。ChatGPT/Claude等
  お好みのAIチャットにコピー&ペーストするだけで使えるのが基本フローで、サーバー側の仲介も
  APIキー登録も不要です。Anthropic/OpenAIの自分のAPIキーで直接生成するBYOK方式も上級者向けに
  用意しています(詳細は後述)
...
(Revolve)があります。ねじフィーチャーではISO並目(M3〜M12)のプリセットから雄ねじ・雌ねじを
配置できます。実体はどちらも簡易表現(雄ねじは呼び径の単純円柱、雌ねじは規格の下穴径
[呼び径-ピッチ]の穴)にとどめ、見た目上のねじらしさはJIS製図の「二重円」表現を参考にした
線オーバーレイ(端面の谷径/呼び径の円+軸方向のヘリックス線)で表示するため、再評価は長さに
かかわらず高速です。配置位置は数値入力/クリックのほか、配置面と同じ面上のスケッチに置いた円を
参照する方式も選べます(押し出さない円は形状に影響しません)。ねじの位置決めは面上スケッチの
円+寸法で行うのが推奨です(寸法変更にねじの位置が自動追従するため)。
...
φ20の穴」)でモデルを生成できます。**基本の使い方はコピー&ペースト(APIキー不要)です**:

1. パネルの「プロンプト仕様をコピー」でプロンプト仕様をクリップボードにコピー
2. ChatGPT・Claudeなどお好みのAIチャットに貼り付けて要望を伝える
3. 返ってきたJSONをパネル下部の貼り付け欄に貼り付けて「読み込む」

この経路はサーバー側の仲介もAPIキー登録も一切不要です。AIチャットには、モデル本体に加えて
ギャラリー投稿用のタイトル/説明/タグの提案も一緒に出力させるため、読み込み後に「ギャラリーに
投稿」を開くとそれらが自動的に入力欄へ反映されます(自由に編集できます)。

- 生成されたモデルは通常のフィーチャーツリーとして読み込まれ、以降は数値編集・寸法駆動編集など
  既存の機能でそのまま編集できます。
- LLMが出力する形式は内部の`CadDocument`ではなく、専用の「アウソリングJSON」
  (`src/ai/authoringSchema.ts`)です。貼り付けテキストの解析(コードフェンス除去・新旧2形式の
  判別、`src/ai/pastePayload.ts`)を経て`src/ai/compile.ts`で検証・変換します。

### APIキーで直接生成(上級者向け、任意)

パネルの「APIキーで直接生成(上級者向け)」を開くと、ブラウザから
[Anthropic API](https://console.anthropic.com/)または[OpenAI API](https://platform.openai.com/)を
直接呼び出す方式(BYOK、Bring Your Own Key)でも生成できます。コピー&ペースト経由と異なり
その場で生成が完了し、寸法が曖昧な指示には質問モードで確認しながら進められます。
...
- 生成→検証(`src/ai/compile.ts`)→スケッチ拘束求解→評価のいずれかに失敗した場合、エラー内容を
  AIへ伝えて自動的に再生成します(最大3回)。両プロバイダとも構造化出力(JSON Schema)でこの
  スキーマに従ったJSONを強制します。
