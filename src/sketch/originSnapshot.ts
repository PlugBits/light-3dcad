// 原点系拘束(distanceEntityOrigin/distancePointOrigin/distanceLineOrigin/coincidentOrigin)のoriginLocalスナップショットを
 ...
 * distanceLineOrigin/coincidentOrigin)のoriginLocalスナップショットを、最新のsketchPlanes(worker評価応答、そのスケッチの
...
export function updateOriginSnapshots(doc: CadDocument, sketchPlanes: readonly SketchPlaneInfo[]): CadDocument {
  ...
  const features = doc.features.map((feature) => {
    ...
    const nextConstraints = (feature as SketchFeature).constraints!.map((c) => {
      if (
        c.kind !== "distanceEntityOrigin" &&
        c.kind !== "distancePointOrigin" &&
        c.kind !== "distanceLineOrigin" &&
        c.kind !== "coincidentOrigin"
      )
        return c;
