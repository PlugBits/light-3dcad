...
## Phase 46: ユーザー要望3件(ねじのスケッチ参照配置・押し出し選択時のハイライト・トースト色分け)

`ThreadFeature.positionRef`(面上スケッチの円参照、省略可)を追加し、evaluator.tsが配置面と
同じ面のスケッチ円中心を面ローカル座標として解決してpositionへ書き戻す(円は押し出さなければ
形状に無害、寸法変更に自動追従)。押し出し/回転体を選択すると、実際に作用したボディ
(`bodyOperationTargets`)をアクセント色でティントし、元スケッチの輪郭も一時強調する。
`showTransientMessage`に成功(緑)/失敗(赤)/情報(濃紺)のvariantを追加し全呼び出し元を分類した。
