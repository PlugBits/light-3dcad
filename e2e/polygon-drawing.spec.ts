...
// 初期ドキュメントには既にNew Bodyの押し出し(Extrude1)があるため単一ボディ制約に抵触する。
// それを避けるため、新規スケッチの押し出しはCut(初期ボックスをZ方向に貫通させる穴)にする
...
test("線描画モードでクリックして矩形を描き、閉じて押し出し(Cut)できる", async ({ page }) => {
  ...
  // 押し出し追加(デフォルトoperationはnewBodyであり、既にボディが存在するため一旦エラーになる)。
